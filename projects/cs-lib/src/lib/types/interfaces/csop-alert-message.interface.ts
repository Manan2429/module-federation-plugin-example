...
export interface ICsopAlertMessage {
  // alert component placeholder where alert should be displayed.
  alertComponentId: string;
  // timeout for self-close
  autoCloseTimer?: number;
  // element that decides flow and is injected in alert custom element slot
  decisionElement?: HTMLElement;
  // if the message is rich text or not
  isRichText?: boolean;
  // message to be displayed on alert
  message: string;
  // type of alert- Refer AlertMessageType enum for values
  type: string;
}
